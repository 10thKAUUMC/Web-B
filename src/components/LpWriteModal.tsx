      ...
      <div className="bg-[#1a1a1e] rounded-2xl w-full max-w-md p-8 relative">
          ...
          <div className="relative flex items-center">
            {thumbnail && (
              <div className="w-32 h-32 rounded-lg overflow-hidden border-2 border-[#333] z-10">
                <img src={thumbnail} alt="LP" className="w-full h-full object-cover" />
              </div>
            )}
            <div
              onClick={() => fileInputRef.current?.click()}
              className={`w-32 h-32 rounded-full bg-[#111] border-4 border-[#222] flex items-center justify-center cursor-pointer hover:border-pink-500 transition-all shadow-xl ${thumbnail ? '-ml-8 z-0' : 'z-10'}`}
            >
        ...
        <div className="flex flex-col gap-3">
          <input
            ...
            className="w-full bg-[#111] border border-[#333] text-white rounded-lg px-4 py-3 focus:outline-none focus:border-pink-500"
          />
          <input
            type="text"
            value={content}
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-5 py-3 bg-[#2a2a2d] hover:bg-pink-500 text-white rounded-lg font-semibold transition-colors"
            >
              Add
            </button>
          </div>
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="flex items-center gap-1 px-3 py-1 bg-[#2a2a2d] text-gray-300 rounded-full text-sm"
                >
                  {tag}
                  <button
